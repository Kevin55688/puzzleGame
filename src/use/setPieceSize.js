export default function setPieceSize (uploadImgData,puzzle,col,row,padding,puzzleBorder,pieceBoder) {
    // 取得原始圖片及puzzle寬度大小
    ...
    const originRatio = ((puzzle.value.offsetWidth - (documentWidth * (PADDING * 0.01) * 2) - (PUZZLE_BORDER * 2))/ imgWidth)
    let CANVAS_WIDTH = (imgWidth * originRatio/ col )  - (PIECE_BORDER * 2)
    let CANVAS_HEIGHT = (imgHeight * originRatio / row)

    console.log(puzzle.value.offsetWidth)
